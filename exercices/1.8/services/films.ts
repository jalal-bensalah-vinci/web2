...
const updateOne = (id: number, updatedFilm : Partial<NewFilm>): Film | undefined => {
  ...
  const film = {...films[index], ...updatedFilm}; //on déclare un film ayant toutes les proprietes de film[index] puis on ajoute/ecrase ceux de updatedFilm
...
const updateOrCreateOne = (id: number, updatedFilm: NewFilm) => {
  const films = parse(jsonDbPath, defaultFilms);
  const index = films.findIndex((film) => film.id === id);
...
const nextId = () =>
  parse(jsonDbPath, defaultFilms).reduce(
    (maxId, film) => Math.max(maxId, film.id), 0
  ) + 1;
